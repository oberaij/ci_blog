...
class MY_Model extends CI_Model
{
    ...
    private function _fetch_table()
    {
        ...
        if ($this->table == NULL)
        {
        $this->table = preg_replace('/(_m|_model|_mdl|model)?$/', '', strtolower(get_class($this)));
        }
    ...
    function __get_columns () {
        ...
        if ( $this->table ) {
            ...
            $_sql   =   "SHOW COLUMNS FROM {$this->table}";
    ...
    public function get_dynamic($params=array(), $count=FALSE, $debug=FALSE) {
        ...
        if (is_array($params['select'])) {
            $this->$db->select(implode(", ", $params['select']));
        } else {
        ...
        if (!empty($params['orwhere'])) {
            foreach ($params['orwhere'] as $key => $item) {
                $this->$db->or_where($key, $item);
            }
        }
        if (!empty($params['like'])) {
            foreach ($params['like'] as $row) {
                $this->$db->or_like($row[0], $row[1], $row[2]);
            }
        }
        if (!empty($params['order'])) {
            foreach ($params['order'] as $item) {
                $this->$db->order_by($item[0], $item[1]);
            }
        }

        $table = !empty($params['table']) ? $table = $params['table'] : $this->table;
        ...
        if ($debug) {
            ...
            echo $timeend . '<br>';
            echo "execution time: " . ($timeend - $timestart) . '<br>';
            ...
            if ($count) {
                ...
                if (!empty($params['limit']) && !empty($params['limit'][1])) {
                    $this->$db->limit($params['limit'][0], $params['limit'][1]);
                } elseif (!empty($params['limit']) && empty($params['limit'][1])) {
